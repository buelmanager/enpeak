...
6. 상태 업데이트:
   ```bash
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase review_outlines --status completed
   ```
...
7. 상태 업데이트:
   ```bash
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase write --status in_progress
   ```
...
9. 모든 글 작성 완료 후 상태 업데이트:
   ```bash
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase write --status completed
   ```
