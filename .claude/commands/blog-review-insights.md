...
1. 최신 run 디렉토리 확인:
   ```bash
   python3 homepage/blog/scripts/latest_run.py
   ```
...
6. 상태 업데이트 + 토픽 등록:
   ```bash
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase review_insights --status completed
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --add-topic {slug} --topic-title "{title}"
   ```
...
7. 상태 업데이트:
   ```bash
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase research --status in_progress
   ```
...
9. 모든 리서치 완료 후 상태 업데이트:
   ```bash
   python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase research --status completed
   ```
...
10. 상태 업데이트:
    ```bash
    python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase outline --status in_progress
    ```
...
12. 모든 개요 완료 후 상태 업데이트:
    ```bash
    python3 homepage/blog/scripts/update_status.py --run-dir {run_dir} --phase outline --status completed
    ```
